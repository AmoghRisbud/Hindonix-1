// MySQL-backed API client
// Replaces the former Upstash Redis client.
// All operations proxy through the Express backend at /api/...
...
export const getHeroImagesFromRedis = (): Promise<string[]> =>
  apiFetch<string[]>("/hero-images").catch(() => ["/images/home/hero-knobs.jpg"]);

export const setHeroImagesInRedis = (urls: string[]): Promise<void> =>
  apiFetch<string[]>("/hero-images", { method: "PUT", body: JSON.stringify({ urls }) }).then(() => undefined);
...
export const migrateLocalStorageToRedis = async (): Promise<{ products: number; caseStudies: number }> => {
  return { products: 0, caseStudies: 0 };
};
