...
const Products = () => {
  ...
  const [currentStep, setCurrentStep] = useState<BrowsingStep>(searchParams.get("step") as BrowsingStep || "category");
  ...
  const [selectedFinish, setSelectedFinish] = useState<string | null>(searchParams.get("finish"));
  ...
  const handleSubcategoryClick = (subcategory: string) => {
    ...
    setSearchParams({ step: "material", category: selectedCategory!, subcategory });
  };
  ...
  const handleMaterialClick = (material: string) => {
    ...
    const params: Record<string, string> = { step: "finish", category: selectedCategory!, material };
  ...
  const handleFinishClick = (finish: string) => {
    ...
    const params: Record<string, string> = { step: "products", category: selectedCategory!, material: selectedMaterial!, finish };
    if (selectedSubcategory) params.subcategory = selectedSubcategory;
    setSearchParams(params);
  };
  ...
  const filteredProducts = products.filter((product) => {
    ...
    if (selectedFinish && !product.finishes.includes(selectedFinish)) return false;
          ...
          {(selectedCategory || selectedSubcategory || selectedMaterial || selectedFinish) && (
            <div className="mb-8 flex items-center gap-2 text-sm text-muted-foreground flex-wrap">
                ...
                <>
                  ...
                  <button
                    onClick={() => {
                      ...
                      setSelectedMaterial(null);
                      setSelectedFinish(null);
                      setCurrentStep(selectedCategory === "Door Handle" ? "subcategory" : "material");
                    }}
                    className="hover:text-accent transition-colors"
                  >
                    {selectedCategory}
              ...
              {selectedFinish && (
                <>
                  <ChevronRight className="w-4 h-4" />
                  <span className="text-foreground font-medium">{selectedFinish}</span>
                </>
              )}
            </div>
          )}
              ...
              {filteredProducts.length > 0 ? (
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {filteredProducts.map((product, index) => {
                    ...
                    const productUrl = `/products/${product.id}?from=${encodeURIComponent(location.pathname + location.search)}`;
